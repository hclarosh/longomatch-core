...
namespace Tests.Controller
{
	public class TestLMTeamTaggerController
	{
		...
		[Test]
		public async Task TaggingController_LineupChange_EventTaggedOnVideoPlayerTime ()
		{
			await AnalysisSetUpAsync ();
			teamTagger.SubstitutionMode = true;
			await App.Current.EventsBroker.Publish (new CapturerTickEvent { Time = new Time (0) });
			var clickedPlayer1 = teamTagger.HomeTeam.FieldPlayersList.FirstOrDefault ();
			var clickedPlayer2 = teamTagger.HomeTeam.BenchPlayersList.FirstOrDefault ();
			...
			Assert.AreEqual (videoPlayer.CurrentTime, projectVM.Timeline.FullTimeline.Model [4].EventTime);
		}
